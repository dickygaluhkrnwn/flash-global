...
import { 
  ...
  ArrowUpDown, Activity, Wallet, History
} from "lucide-react";
import { db } from "@/lib/firebase";
import { collection, getDocs, doc, updateDoc, serverTimestamp, increment, addDoc } from "firebase/firestore";
...
// =========================================================================
// CUSTOM STYLES: APPLE GLASSMORPHISM (Emerald Accent)
// =========================================================================
...
export default function AdminWalletDriversPage() {
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-black text-slate-900 tracking-tight flex items-center gap-2">
              Buku Kas Mitra (Drivers)
            </h1>
            <p className="text-slate-500 text-sm mt-0.5 font-medium flex items-center gap-2">
              Manajemen saldo bagi hasil dan komisi. <AdminBadge variant="success">Finance Hub</AdminBadge>
            </p>
          </div>
        </div>
      </div>
        ...
        <div className="xl:col-span-4 space-y-6 lg:sticky lg:top-24">
          <div className="bg-gradient-to-br from-slate-900 to-slate-800 rounded-[2rem] p-8 shadow-[0_20px_40px_rgba(0,0,0,0.15)] relative overflow-hidden">
            ...
            <p className="text-emerald-400 text-xs font-bold uppercase tracking-widest mb-2 relative z-10 flex items-center gap-2">
              <Wallet className="w-4 h-4" /> Total Aset Mengendap
            </p>
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center gap-4 w-full lg:w-[40%]">
                        <div className="w-12 h-12 bg-slate-100 text-slate-500 rounded-xl flex items-center justify-center border border-slate-200 shrink-0 shadow-sm">
                          <UserCircle className="w-6 h-6" />
                        </div>
                        <div className="overflow-hidden">
                          <h2 className="text-sm font-black text-slate-900 truncate tracking-tight">{item.name}</h2>
                          <p className="text-[11px] font-bold text-slate-500 mt-0.5 font-mono">{item.phone}</p>
                          <AdminBadge variant={item.partnerType === "Vendor" ? "warning" : "info"} className="mt-1.5 shadow-sm text-[8px] tracking-widest">
                            {item.partnerType === "Vendor" ? "Vendor / Fleet Manager" : "Sopir Mandiri"}
                          </AdminBadge>
                        </div>
                      </div>
                      ...
                      <div className="w-full lg:w-[30%] flex flex-col items-start lg:items-end gap-1 border-t border-slate-100 pt-4 lg:pt-0 lg:border-t-0">
                        <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-0.5">Saldo Mengendap</p>
                      ...
                      <div className="w-full lg:w-[30%] flex items-center justify-start lg:justify-end gap-2 border-t border-slate-100 pt-4 lg:pt-0 lg:border-t-0">
                        <AdminButton 
                          ...
                          className="h-10 border-emerald-200 text-emerald-600 hover:bg-emerald-50 hover:border-emerald-300 shadow-sm font-bold w-full lg:w-auto"
                        >
                        ...
                        >
                          <ArrowDownCircle className="w-4 h-4 mr-1.5" /> Tarik
                        </AdminButton>
                      </div>
                    </motion.div>
                  );
