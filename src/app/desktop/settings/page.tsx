...
import { 
  ...
  Mail, Phone, Camera, Crown, CheckCircle2, 
...
export default function DesktopSettingsPage() {
  ...
  // STATE KONTROL MODE EDIT (TIDAK TERBUKA SECARA DEFAULT)
  ...
  // Fetch Data Ekstra dari Firestore saat komponen dimuat
  useEffect(() => {
    if (user?.uid) {
  ...
  const handleSaveProfile = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setIsSuccess(true);
      setIsEditingProfile(false); // Kembalikan ke Read Mode setelah sukses
  ...
  const handleSaveCompany = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setIsSuccess(true);
      setIsEditingCompany(false); // Kembalikan ke Read Mode setelah sukses
      setTimeout(() => setIsSuccess(false), 4000);
    } catch (error) {
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <div className="w-full lg:w-1/4 shrink-0">
            <div className="bg-white rounded-3xl shadow-xl shadow-[#7A171D]/5 p-4 border border-gray-100 flex flex-col space-y-2 sticky top-24">
              <button 
                onClick={() => { setActiveTab("profil"); setIsEditingProfile(false); }}
              ...
              <button 
                onClick={() => { setActiveTab("perusahaan"); setIsEditingCompany(false); }}
                className={`flex items-center gap-3 px-4 py-3.5 rounded-xl text-sm font-bold transition-all ${
              ...
              <button 
                ...
                className={`flex items-center gap-3 px-4 py-3.5 rounded-xl text-sm font-bold transition-all ${
                  activeTab === "b2b" ? "bg-gradient-to-r from-[#C5A059] to-[#DFBE7B] text-gray-900 shadow-md shadow-[#C5A059]/30" : "text-gray-600 hover:bg-slate-50"
                }`}
              >
              ...
              <button 
                ...
                className={`flex items-center gap-3 px-4 py-3.5 rounded-xl text-sm font-bold transition-all ${
                  activeTab === "keamanan" ? "bg-gray-900 text-white shadow-md" : "text-gray-600 hover:bg-slate-50"
                }`}
              >
          ...
          <div className="w-full lg:w-3/4">
            ...
            <motion.div
              ...
              className="bg-white rounded-3xl shadow-xl shadow-[#7A171D]/5 p-6 md:p-10 border border-gray-100"
            >
              ...
              {activeTab === "profil" && (
                <div className="space-y-8">
                  <div className="flex items-center justify-between border-b pb-5 border-gray-100">
                    ...
                    {!isEditingProfile && (
                      <button 
                        ...
                        className="bg-slate-100 hover:bg-slate-200 text-gray-800 font-bold px-4 py-2.5 rounded-xl text-xs flex items-center gap-2 transition-colors"
                      >
                  ...
                  {!isEditingProfile ? (
                    /* VIEW MODE (TAMPILAN BACA SAJA YANG RAPI) */
                    <div className="space-y-8">
                    ...
                    <form onSubmit={handleSaveProfile} className="space-y-6 animate-fadeIn">
                      <div className="flex flex-col sm:flex-row items-center gap-6 p-5 bg-amber-50/50 rounded-2xl border border-amber-100">
                        <div className="relative group">
                          ...
                          <button 
                            ...
                            className="absolute bottom-0 right-0 w-8 h-8 bg-[#C5A059] rounded-full border-2 border-white flex items-center justify-center text-white hover:bg-[#b08d4a] transition-colors shadow"
                          >
                        ...
                        <div>
                          ...
                          <p className="text-xs text-gray-500 mt-1">Format JPG atau PNG. Ukuran maksimal 2MB.</p>
                        </div>
                      </div>
                      ...
                      <div className="pt-6 border-t border-gray-100 flex items-center justify-end gap-3">
                        <button 
                          ...
                          className="bg-gray-100 hover:bg-gray-200 text-gray-700 font-bold px-6 py-3 rounded-xl text-sm flex items-center gap-2 transition-colors"
                        >
                        ...
                        <button 
                          type="submit" 
                          disabled={isLoading}
                          className="bg-[#7A171D] hover:bg-[#5A0E13] text-white font-bold px-6 py-3 rounded-xl text-sm flex items-center gap-2 transition-all shadow-md disabled:opacity-70"
                        >
                  ...
                  {!isEditingCompany ? (
                    ...
                    <div className="grid grid-cols-1 gap-6">
                      <div className="p-6 rounded-2xl border border-gray-100 space-y-1 bg-slate-50/60 shadow-sm flex items-start gap-4">
                        <div className="w-12 h-12 bg-white rounded-xl flex items-center justify-center border border-gray-100 text-[#7A171D] shrink-0">
                      ...
                      <div className="p-6 rounded-2xl border border-gray-100 space-y-2 bg-slate-50/60 shadow-sm flex items-start gap-4">
                        <div className="w-12 h-12 bg-white rounded-xl flex items-center justify-center border border-gray-100 text-[#C5A059] shrink-0">
                          <MapPin className="w-6 h-6" />
                        </div>
                        <div className="flex-1">
                          <span className="text-xs font-bold text-gray-400 uppercase">Alamat Gudang / Penjemputan Default</span>
                          <p className="font-bold text-gray-800 text-sm mt-1 leading-relaxed whitespace-pre-line">
                            {formData.defaultAddress || "Belum ada alamat penjemputan utama yang diatur."}
                          </p>
                        </div>
                      </div>
                    </div>
                      ...
                      <div className="grid grid-cols-1 gap-6">
                        ...
                        <div className="space-y-2">
                          <label className="text-sm font-semibold text-gray-700 flex justify-between">
                            Alamat Penjemputan / Gudang Default
                            <span className="text-xs text-gray-400 font-normal">Akan otomatis terisi di form booking</span>
                          </label>
              ...
              {activeTab === "b2b" && (
                <div className="space-y-6">
                  <div className="bg-gradient-to-br from-gray-900 to-slate-800 rounded-3xl p-8 text-white relative overflow-hidden">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 relative z-10 mb-8">
                      <div className="flex items-start gap-3">
                        ...
                        <span className="text-sm font-medium">Pembayaran Tempo (Sistem Invoice)</span>
                      </div>
                      <div className="flex items-start gap-3">
                        <CheckCircle2 className="w-5 h-5 text-[#C5A059] shrink-0" />
                        <span className="text-sm font-medium">Harga Grosir / Diskon Volume Khusus</span>
                      </div>
                      <div className="flex items-start gap-3">
                        <CheckCircle2 className="w-5 h-5 text-[#C5A059] shrink-0" />
                        <span className="text-sm font-medium">Prioritas Armada & Customer Service</span>
                      </div>
                      <div className="flex items-start gap-3">
                        <CheckCircle2 className="w-5 h-5 text-[#C5A059] shrink-0" />
                        <span className="text-sm font-medium">Laporan Manifes & Analitik Bulanan</span>
                      </div>
                    </div>

                    <button 
                      type="button"
                      onClick={handleUpgradeB2B}
                      disabled={isLoading}
                      className="bg-gradient-to-r from-[#C5A059] to-[#DFBE7B] hover:from-[#b08d4a] hover:to-[#C5A059] text-gray-900 font-bold px-8 py-4 rounded-xl flex items-center gap-2 transition-all shadow-lg shadow-[#C5A059]/20 disabled:opacity-70 disabled:cursor-not-allowed"
                    >
                  ...
                  <div className="bg-amber-50 border border-amber-100 p-4 rounded-2xl text-amber-800 text-xs leading-relaxed flex gap-3">
                    ...
                    <p>
                      <strong>Verifikasi Dokumen:</strong> Setelah pengajuan dikirim, tim Legal Flash Global akan menghubungi Anda untuk meminta dokumen legalitas perusahaan (NIB, NPWP, Akta) sebagai syarat validasi akun korporat.
                    </p>
                  </div>
                </div>
              )}
