...
import { 
  ...
  Send, ChevronRight, HelpCircle, ChevronDown, BookOpen
} from "lucide-react";
...
export default function DesktopSupportPage() {
  ...
  const fetchMyTickets = async () => {
    ...
    try {
      ...
      console.error("Gagal menarik tiket dukungan:", error);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleSubmitTicket = async (e: React.FormEvent) => {
    ...
    try {
      ...
      fetchMyTickets(); // Refresh daftar tiket
    } catch (error) {
  ...
  return (
    <main className="min-h-screen bg-slate-50 py-10 px-4 md:px-8 relative overflow-hidden font-sans pb-24">
      ...
      <div className="max-w-[1200px] mx-auto relative z-10 space-y-8">
        ...
        {/* HEADER HERO */}
        <div className="bg-white rounded-[2rem] border border-slate-200 shadow-sm p-8 md:p-12 relative overflow-hidden">
          ...
          <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-8">
            <div className="max-w-2xl">
              <Badge variant="brand" className="uppercase text-[10px] px-3 py-1 shadow-sm mb-4">Pusat Layanan Klien</Badge>
              <h1 className="text-3xl md:text-5xl font-black text-slate-900 tracking-tight flex items-center gap-3 mb-4">
                <LifeBuoy className="w-10 h-10 text-blue-600" /> Bantuan & FAQ
              </h1>
              <p className="text-slate-500 font-medium leading-relaxed text-base">
                Temukan jawaban cepat untuk kendala logistik Anda di bawah ini, atau buat tiket langsung untuk ditangani oleh Tim Customer Success kami.
              </p>
            </div>
            ...
            <div className="shrink-0 flex flex-col gap-3">
              <Button 
                ...
                className="w-full md:w-auto bg-[#7A171D] hover:bg-[#5A0E13] text-white px-8 h-14 rounded-2xl shadow-lg shadow-[#7A171D]/20 font-bold text-sm flex items-center justify-center gap-2"
              >
                <PlusCircle className="w-5 h-5" /> Buat Tiket Bantuan
              </Button>
              <p className="text-[10px] font-bold text-slate-400 text-center uppercase tracking-widest">Waktu Respons: &lt; 2 Jam</p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
                ...
                return (
                  <div key={idx} className="border-b border-slate-100 last:border-0">
                    <button 
                      ...
                      className={cn(
                        "w-full flex items-center justify-between text-left p-4 md:p-5 transition-colors outline-none",
                    ...
                    <AnimatePresence initial={false}>
                      {isOpen && (
                        ...
                        >
                          <div className="p-4 md:p-5 pt-0 text-xs md:text-sm text-slate-600 font-medium leading-relaxed bg-blue-50/50">
                            {faq.a}
                          </div>
                        </motion.div>
                      )}
          ...
          <div className="lg:col-span-7 space-y-6">
            <h2 className="text-lg font-black text-slate-900 flex items-center gap-2 px-2">
              <MessageSquare className="w-5 h-5 text-slate-400" /> Riwayat Tiket Anda
            </h2>
            ...
            <div className="space-y-4">
              {isLoading ? (
                <div className="flex flex-col items-center justify-center min-h-[300px] bg-white rounded-2xl border border-slate-200 shadow-sm">
                  ...
                  <p className="text-slate-400 font-bold animate-pulse text-sm uppercase tracking-widest">Memuat Riwayat...</p>
                </div>
              ) : tickets.length === 0 ? (
                <div className="flex flex-col items-center justify-center min-h-[300px] bg-white rounded-2xl border border-slate-200 shadow-sm text-center px-6">
                  ...
                  <p className="text-slate-500 font-medium max-w-sm mb-8 text-sm">Anda belum pernah mengajukan tiket bantuan. Jika butuh sesuatu atau FAQ tidak membantu, silakan klik tombol Buat Tiket Bantuan.</p>
                </div>
              ) : (
                tickets.map((ticket) => (
                  <div key={ticket.id} className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 hover:shadow-md transition-shadow group flex flex-col sm:flex-row gap-6 justify-between sm:items-center relative overflow-hidden">
                    
                    {/* Status Indikator Dekoratif */}
                    <div className={`absolute top-0 left-0 bottom-0 w-1.5 ${ticket.status === 'Resolved' ? 'bg-emerald-400' : ticket.status === 'In Progress' ? 'bg-amber-400' : 'bg-slate-300'}`} />

                    <div className="space-y-3 w-full">
                      <div className="flex flex-wrap items-center gap-3">
                        <span className="font-mono font-black text-slate-900 text-xs md:text-sm uppercase bg-slate-100 px-2.5 py-1 rounded-md border border-slate-200">
                        ...
                        <span className={`px-2.5 py-0.5 rounded text-[9px] font-black uppercase tracking-widest border ml-auto sm:ml-0 ${
                    ...
                    <div className="flex flex-row sm:flex-col items-center sm:items-end justify-between sm:justify-center gap-2 shrink-0 border-t sm:border-t-0 sm:border-l border-slate-100 pt-4 sm:pt-0 sm:pl-6 w-full sm:w-auto">
                      <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest block sm:hidden">Status Tiket</span>
                      <span className={`px-4 py-2 rounded-xl text-xs font-black uppercase tracking-widest border inline-flex items-center justify-center gap-2 w-full sm:w-36 text-center ${
                        ticket.status === 'Resolved' ? 'bg-emerald-50 text-emerald-600 border-emerald-200 shadow-sm' :
                        ticket.status === 'In Progress' ? 'bg-amber-50 text-amber-600 border-amber-200 shadow-sm' :
            ...
            <motion.div initial={{ scale: 0.95, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.95, opacity: 0, y: 20 }} className="relative w-full max-w-xl bg-white rounded-[2rem] p-8 shadow-2xl border border-slate-100">
              ...
              <div className="flex justify-between items-start mb-6 border-b border-slate-100 pb-5">
                <div>
                  ...
                  <p className="text-xs text-slate-500 mt-1.5 font-medium">Lengkapi formulir di bawah agar tim kami dapat memahami kendala Anda.</p>
                </div>
              ...
              <form onSubmit={handleSubmitTicket} className="space-y-6">
                ...
                <div className="space-y-2">
                  ...
                  <textarea 
                    ...
                    required
                    rows={4}
                    className="flex w-full rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm font-medium text-slate-900 transition-all focus-visible:outline-none focus-visible:ring-4 focus-visible:border-blue-500 focus-visible:bg-white focus-visible:ring-blue-50 resize-none"
                  />
                ...
                <div className="pt-4 border-t border-slate-100 flex gap-3 justify-end">
                  <Button type="button" onClick={() => setShowModal(false)} variant="outline" className="px-6 border-slate-200 bg-white">Batal</Button>
                  <Button type="submit" disabled={isSubmitting} className="w-full sm:w-auto bg-blue-600 hover:bg-blue-700 text-white font-bold px-8 shadow-md shadow-blue-600/20 border-none flex items-center justify-center gap-2">
