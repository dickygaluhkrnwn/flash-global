...
export default function DeliveryOrderDetailPage() {
  ...
  const formatFirebaseDate = (timestamp: FirebaseTimestamp | Date | string | number | null | undefined) => {
    ...
    if (timestamp instanceof Date) {
      ...
      millis = new Date(timestamp as string | number).getTime();
    }
  ...
  const renderTimeline = (): TimelineItem[] => {
    ...
    if (order.trackingHistory && Array.isArray(order.trackingHistory) && order.trackingHistory.length > 0) {
      return [...order.trackingHistory].reverse().map((item, idx) => ({ 
        ...
        description: String(item.description || "Status diperbarui"),
        location: String(item.location || "Sistem")
      }));
    }
    return [{
      status: String(order.status || "Menunggu Pembayaran"),
      description: String(order.statusSub || order.paymentStatus || "Pesanan telah diterima oleh sistem dan menunggu proses."),
  ...
  if (errorMsg || !order) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center font-sans px-6 text-center bg-[#f8fafc] relative z-0">
        ...
        <div className="bg-white/80 backdrop-blur-xl p-10 md:p-12 rounded-[3rem] border border-white flex flex-col items-center max-w-lg shadow-[0_20px_60px_rgba(0,0,0,0.05)]">
  ...
  const originAddress = String(originObj?.address || (typeof order.origin === 'string' ? order.origin : "-"));
  const originName = String(originObj?.senderName || order.senderName || order.name || "-");
  const originPhone = String(originObj?.senderPhone || order.senderPhone || "-");

  let destAddress = String(order.destination || "-");
  let destName = "Penerima";
  let destPhone = "-";

  if (order.destinations && order.destinations.length > 0) {
    destAddress = String(order.destinations.length > 1 ? `${order.destinations.length} Titik Tujuan` : (order.destinations[0].address || "Tujuan"));
    destName = String(order.destinations[0].receiverName || "Penerima");
    destPhone = String(order.destinations[0].receiverPhone || "-");
  }

  const isMoneyInvolved = order.paymentStatus === "Lunas" || order.paymentStatus === "Menunggu Verifikasi Finance" || order.paymentStatus === "Piutang B2B"; 
  const isEligibleForClaim = order.breakdown && order.breakdown.insuranceFee > 0; 
  const maxClaimAllowed = order.totalItemValue ? Number(order.totalItemValue) : (Number(order.breakdown?.deliveryFee) || 0) * 10;
  ...
  const invoiceDiscount = Number(order.breakdown?.b2bDiscount || 0) + Number(order.discountPromoAmount || 0);
  const invoiceGrandTotal = Number(order.finalGrandTotal || order.breakdown?.grandTotal || order.totalCost || 0);
    ...
    <main className="min-h-screen bg-[#f8fafc] py-12 lg:py-20 px-6 relative overflow-hidden font-sans pb-32 z-0">
      ...
      <div className="max-w-[1200px] mx-auto relative z-10 space-y-6">
        ...
        {/* ==========================================
            ...
            ========================================== */}
        <div className="bg-white/70 backdrop-blur-[40px] saturate-[180%] border border-white shadow-[0_8px_30px_rgba(0,0,0,0.04),inset_0_1px_1px_rgba(255,255,255,1)] transition-all duration-300 rounded-[2rem] p-6 md:p-10 flex flex-col md:flex-row justify-between md:items-center gap-8 relative overflow-hidden">
          ...
          <div className="relative z-10">
            <div className="flex flex-wrap items-center gap-3 mb-4">
              <Badge variant="primary" className="shadow-sm px-3.5 py-1.5 text-[10px] uppercase tracking-widest font-black">{String(order.serviceType || "Kargo Reguler")}</Badge>
        ...
        {/* ==========================================
            MAIN CONTENT GRID (NATURAL FLOW)
            ========================================== */}
        <div className="flex flex-col gap-6 lg:gap-8 mt-6">
          
          {/* ROW 1: Rute & Cargo Specs (Kiri) VS Billing & Support (Kanan) */}
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8 items-start">
            ...
            <div className="lg:col-span-7 w-full">
              ...
              <div className="bg-white/70 backdrop-blur-[40px] saturate-[180%] border border-white shadow-[0_8px_30px_rgba(0,0,0,0.04),inset_0_1px_1px_rgba(255,255,255,1)] transition-all duration-300 rounded-[2.5rem] p-6 md:p-8 relative overflow-hidden h-full flex flex-col justify-between">
                          ...
                          {order.destinations && order.destinations.length > 0 ? order.destinations.map((dest: LocationDetail, idx: number) => (
                            <div key={idx} className="bg-white/60 backdrop-blur-md p-6 rounded-[1.5rem] border border-white shadow-[inset_0_2px_4px_rgba(255,255,255,0.8)] mb-4 last:mb-0 relative overflow-hidden group">
                              ...
                              <div className="flex justify-between items-start mb-2 relative z-10">
                                <p className="font-bold text-slate-900 text-sm leading-relaxed pr-4 break-words line-clamp-2">{String(dest.address || "-")}</p>
                              ...
                              {dest.receiverName && (
                                <div className="mt-5 pt-4 border-t border-slate-200/60 flex flex-wrap items-center justify-between gap-3 text-xs font-black text-slate-500 relative z-10">
                                  <span className="flex items-center gap-2 min-w-0"><User className="w-4 h-4 shrink-0 text-slate-400"/> <span className="truncate">{String(dest.receiverName)}</span></span>
                                  <span className="flex items-center gap-2 shrink-0 bg-white px-3 py-1.5 rounded-lg border border-slate-100 shadow-sm"><Phone className="w-3.5 h-3.5 text-slate-400"/> {String(dest.receiverPhone || "-")}</span>
                                </div>
                              )}
                            </div>
                          )) : (
                            <div className="bg-white/60 backdrop-blur-md p-6 rounded-[1.5rem] border border-white shadow-[inset_0_2px_4px_rgba(255,255,255,0.8)] relative z-10">
                               <p className="font-bold text-slate-900 text-sm leading-relaxed break-words line-clamp-2">{destAddress}</p>
                            </div>
                          )}
                        </div>
                      </div>
                ...
                <div className="pt-8 mt-6 border-t border-slate-200/60 grid grid-cols-1 md:grid-cols-2 gap-5 relative z-10">
                  <div className="bg-white/60 backdrop-blur-md p-5 rounded-[1.5rem] border border-white shadow-[inset_0_2px_4px_rgba(0,0,0,0.02)] flex items-center gap-5 hover:bg-white transition-colors group">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="font-black text-slate-900 text-lg tracking-tight truncate">{Number(order.totalWeight || order.weight || 0)} Kg</p>
                    </div>
                  </div>
                  <div className="bg-white/60 backdrop-blur-md p-5 rounded-[1.5rem] border border-white shadow-[inset_0_2px_4px_rgba(0,0,0,0.02)] flex items-center gap-5 hover:bg-white transition-colors group">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="font-black text-slate-900 text-lg tracking-tight truncate">{String(order.vehicleName || order.vehicle || order.serviceType)}</p>
                    </div>
                  </div>
                    ...
                    <div className="md:col-span-2 bg-gradient-to-r from-slate-800 to-slate-900 p-5 rounded-[1.5rem] border border-slate-700 shadow-xl flex flex-col sm:flex-row sm:items-center justify-between gap-4 group">
                      <div className="flex items-center gap-5 min-w-0">
                        ...
                        <div className="min-w-0">
                          ...
                          <p className="font-black text-white text-lg tracking-tight truncate">{String(order.driverName)}</p>
                        </div>
                      </div>
            ...
            {/* --------------------------------------
                KOLOM KANAN ROW 1
                -------------------------------------- */}
            <div className="lg:col-span-5 w-full flex flex-col gap-6 lg:gap-8 h-full">
                  ...
                  <div className="space-y-5 mb-10 text-sm font-semibold">
                    {order.breakdown ? (
                      <>
                        ...
                        {order.appliedPromoCode && <div className="flex justify-between items-center text-pink-400 border-t border-slate-700/50 pt-5 mt-5"><span className="flex items-center gap-2"><TicketPercent className="w-4 h-4"/> Kode Promo ({String(order.appliedPromoCode)})</span><span className="font-bold">- {formatIDR(order.discountPromoAmount)}</span></div>}
                      </>
                    ) : (
              ...
              <div className="bg-white/70 backdrop-blur-[40px] saturate-[180%] rounded-[2.5rem] p-6 md:p-8 space-y-6 border border-white shadow-[0_15px_40px_rgba(0,0,0,0.03)] flex-1">
          ...
          {/* ROW 2: TIMELINE (KIRI) vs REFUND (KANAN) */}
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8 items-start">
            
            <div className="lg:col-span-7 w-full">
              {/* KARTU TIMELINE */}
              <div className="bg-white/70 backdrop-blur-[40px] saturate-[180%] rounded-[2.5rem] p-6 md:p-8 border border-white shadow-[0_15px_40px_rgba(0,0,0,0.03)]">
                 <OrderTimeline timelineData={timelineData} orderStatus={String(order.status)} />
              </div>
            </div>
            ...
            <div className="lg:col-span-5 w-full">
              ...
              {(hasExistingRefund || order.paymentStatus?.includes("Refund")) && (
                <div className={`p-6 md:p-8 rounded-[2rem] border shadow-[inset_0_2px_4px_rgba(255,255,255,0.8)] ${
                  ...
                  <p className={`text-sm font-semibold leading-relaxed ${(!refundRequestData || refundRequestData?.status === "Pending" || order.paymentStatus === "Menunggu Refund") ? "text-amber-600" : "text-slate-600"}`}>
                    {(!refundRequestData || refundRequestData?.status === "Pending" || order.paymentStatus === "Menunggu Refund") 
                      ...
                      : `Nominal Kembali: ${formatIDR(Number(refundRequestData?.nominal || order.finalGrandTotal || order.totalCost))}`}
                  </p>
                  ...
                  {(refundRequestData?.status === "Approved" || order.paymentStatus === "Refund Selesai") && refundRequestData?.proofUrl && (
                    <div className="mt-6 pt-6 border-t border-emerald-200/60">
                      ...
                      <div className="bg-white p-2.5 rounded-[1.5rem] border border-emerald-100 relative group overflow-hidden flex justify-center shadow-sm">
                        ...
                        <img src={String(refundRequestData.proofUrl)} alt="Bukti Refund" className="w-full h-auto max-h-56 object-cover rounded-xl" />
                        <div className="absolute inset-0 bg-slate-900/40 backdrop-blur-sm opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center">
                          <a href={String(refundRequestData.proofUrl)} target="_blank" rel="noopener noreferrer" className="bg-white text-slate-900 px-6 py-3 rounded-xl font-black text-xs shadow-xl flex items-center gap-2 hover:bg-slate-50 transition-colors active:scale-95">
      ...
      <div className="hidden">
        {order && <ReceiptTemplate ref={receiptRef} resi={resiNumber} senderName={originName} senderPhone={originPhone} originAddress={originAddress} receiverName={destName} receiverPhone={destPhone} destAddress={destAddress} weight={Number(order.totalWeight || order.weight || 0)} serviceType={String(order.serviceType || "Kargo")} vehicleName={String(order.vehicleName || order.vehicle || "Armada")} date={formatFirebaseDate(order.createdAt).split(",")[0]} itemsDesc={String(((order.destinations?.[0]?.items as DeliveryItem[])?.[0]?.name) || "Paket Kargo")} />}
        {order && <InvoiceA4Template ref={invoiceRef} invoiceNumber={`INV-${resiNumber}`} issueDate={issueDateObj.toLocaleDateString('id-ID')} dueDate={dueDateObj.toLocaleDateString('id-ID')} clientName={user?.displayName || originName || "Klien Yth."} clientCompany={user?.companyName} clientAddress={originAddress} clientEmail={user?.email || order.email || "-"} clientPhone={originPhone || user?.phoneNumber || "-"} items={invoiceItems} subTotal={invoiceSubTotal} discountAmount={invoiceDiscount} taxAmount={0} grandTotal={invoiceGrandTotal} />}
      </div>
